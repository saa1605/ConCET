...
class TextCNN(object):
    ...
    embedding_size, embedding_entity_size, embedding_pos_size, embedding_char_size, filter_sizes, num_filters, l2_reg_lambda=0.0):
            ...
            self.input_x_entity = tf.compat.v1.placeholder(tf.int32, [None, sequence_length], name="input_entityvector")
            self.input_x_pos = tf.compat.v1.placeholder(tf.int32, [None, sequence_length], name="input_posvector")
            self.input_x_hand = tf.compat.v1.placeholder(tf.float32, [None, 1], name="input_x_hand")
            self.input_bag_of_entity = tf.compat.v1.placeholder(tf.float32, [None, 1000], name="input_bag_of_entity")
            self.input_x = tf.compat.v1.placeholder(tf.int32, [None, sequence_length], name="input_x")
            self.input_char = tf.compat.v1.placeholder(tf.int32, [None, sequence_char_length], name="input_char")
            ...
            with tf.device('/cpu:0'), tf.compat.v1.name_scope("embedding"):
                self.W = tf.Variable(tf.compat.v1.random_uniform([vocab_size, embedding_size], -1.0, 1.0), name="W", trainable= True)

                # self.place_w = tf.compat.v1.placeholder(tf.float32, shape=(vocab_size, embedding_size))
            ...
            for i, filter_size in enumerate(filter_sizes):
                with tf.compat.v1.name_scope("conv-maxpool-%s" % filter_size):
                    ...
                    conv = tf.nn.conv2d(
                        ...
                        padding="VALID",
                        name="conv")
                    conv = tf.compat.v1.layers.batch_normalization(inputs=conv, momentum=0.997, epsilon=1e-5, center=True, scale=True)
            ...
            # Embedding Lookup 16
            with tf.compat.v1.name_scope("embedding_char"):
                self.embedding_character = tf.compat.v1.get_variable(name='lookup_Wchar',
                                                        shape=[num_quantized_chars, embedding_char_size],
                                                        initializer=tf.keras.initializers.he_uniform(), trainable=True)
                self.embedded_characters = tf.nn.embedding_lookup(params=self.embedding_character, ids=self.input_char)

            # Create a convolution + maxpool layer for each filter size
            with tf.compat.v1.name_scope("rnn_char"):
                with tf.compat.v1.variable_scope('rnn_chr') as scope:
                    cell = self._get_cell(sequence_char_length, 'gru')
                    cell_with_attention = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self.dropout_keep_prob)
                    # cell_with_attention = tf.contrib.rnn.AttentionCellWrapper(cell_with_attention, sequence_char_length)
                    
                    outputs, output_states = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=cell_with_attention,
            ...
            # Embedding Lookup 16
            with tf.compat.v1.name_scope("embedding_entiy"):
                self.embedding_entity = tf.compat.v1.get_variable(name='lookup_Went',
                                                        shape=[vocab_entity_size, embedding_entity_size],
                                                        initializer=tf.keras.initializers.he_uniform(), trainable=True)
                self.embedded_entity = tf.nn.embedding_lookup(params=self.embedding_entity, ids=self.input_x_entity)

            # Create a convolution + maxpool layer for each filter size
            with tf.compat.v1.name_scope("rnn_charent"):
                with tf.compat.v1.variable_scope('rnn_chrent') as scope:
                    cell = self._get_cell(sequence_length, 'gru')
                    cell_with_attention = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self.dropout_keep_prob)
                    # cell_with_attention = tfa.rnn.AttentionCellWrapper(cell_with_attention, sequence_length)
                    outputs, output_states = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=cell_with_attention,
                                                                             cell_bw=cell_with_attention,
            ...
            # Embedding Lookup 16
            with tf.compat.v1.name_scope("embedding_pos"):
                self.embedding_pos = tf.compat.v1.get_variable(name='lookup_W1pos',
                                                        shape=[vocab_pos_size, embedding_pos_size],
                                                        initializer=tf.keras.initializers.he_uniform(), trainable=True)
                self.embedded_pos = tf.nn.embedding_lookup(params=self.embedding_pos, ids=self.input_x_pos)
            # Create a convolution + maxpool layer for each filter size
            with tf.compat.v1.name_scope("rnn_charpos"):
                with tf.compat.v1.variable_scope('rnn_chrpos') as scope:
                    cell = self._get_cell(sequence_length, 'gru')
                    cell_with_attention = tf.compat.v1.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self.dropout_keep_prob)
                    # cell_with_attention = tfa.rnn.AttentionCellWrapper(cell_with_attention, sequence_length)
            ...
            with tf.compat.v1.name_scope("NN_entity_hand"):
                with tf.compat.v1.variable_scope('FCNN_entity_hand') as scope:
                    ...
                    self.dense_entity_hand = tf.compat.v1.nn.xw_plus_b(self.h_pool2_entity_hand, W, b, name="scores_entity_hand")

            with tf.compat.v1.name_scope("dropout_entity_hand"):
                self.h_drop_entity_BOE = tf.nn.dropout(self.dense_entity_hand, rate=1 - (self.dropout_keep_prob))
            ...
            with tf.compat.v1.name_scope("NN_h_pool_flat"):
                with tf.compat.v1.variable_scope('FCNN_h_pool_flat') as scope:
                    W = tf.compat.v1.get_variable(
                        "W_h_pool_flat",
                    ...
                    l2_loss += tf.nn.l2_loss(W)
                    l2_loss += tf.nn.l2_loss(b)
                    self.dense_utt_cnn = tf.compat.v1.nn.xw_plus_b(self.h_pool_utt_cnn_flat, W, b, name="scores_h_pool_flat")

            with tf.compat.v1.name_scope("dropout_h_pool_flat"):
                self.h_drop_cnn_dense_textual = tf.nn.dropout(self.dense_utt_cnn, rate=1 - (self.dropout_keep_prob))

            ####################################################################################
            ####################################################################################

            # self.dot_product = tf.reduce_sum( tf.multiply( self.h_drop_entity_BOE, self.h_drop_cnn_dense_textual ), 1, keepdims=True )
            self.cosine_distance = tf.reduce_sum(input_tensor=tf.multiply(tf.nn.l2_normalize(self.h_drop_entity_BOE, 0), tf.nn.l2_normalize(self.h_drop_cnn_dense_textual, 0)),  axis=1, keepdims=True )
            ...
            with tf.compat.v1.variable_scope('FCNN') as scope:
                ...
                l2_loss += tf.nn.l2_loss(W)
                l2_loss += tf.nn.l2_loss(b)
                self.dense = tf.compat.v1.nn.xw_plus_b(self.h_pool2_flat, W, b, name="scores")
            ...
            with tf.compat.v1.name_scope("dropout"):
                self.h_drop = tf.nn.dropout(self.dense, rate=1 - (self.dropout_keep_prob))
            ...
            with tf.compat.v1.name_scope("accuracy"):
                self.true_labels = tf.math.argmax(self.input_y, 1)
                correct_predictions = tf.math.equal(self.predictions, self.true_labels)
                self.accuracy = tf.math.reduce_mean(input_tensor=tf.cast(correct_predictions, "float"), name="accuracy")

    # @staticmethod
    def _get_cell(self,hidden_size, cell_type):
        if cell_type == "vanilla":
            return tf.compat.v1.nn.rnn_cell.BasicRNNCell(hidden_size)
        elif cell_type == "lstm":
            return tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_size)
        elif cell_type == "gru":
    ...
    def _length(self, seq):
        relevant = tf.math.sign(tf.abs(seq))
        length = tf.math.reduce_sum(input_tensor=relevant, axis=1)
        length = tf.cast(length, tf.int32)
        return length

    # @staticmethod
    def last_relevant(self, seq, length):
        batch_size = tf.shape(input=seq)[0]
